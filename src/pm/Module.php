...
use Phi\Exceptions\ExceptionCode;
...
class Module implements \Stringable
{
    ...
    {
        if (! is_file($file)) {
            throw new Exception("No such file $file", ExceptionCode::NO_SUCH_FILE);
        }
        if (! is_readable($file)) {
            throw Exception("$file is not readable", ExceptionCode::FILE_NOT_READABLE);
        }
    }
