...
use Phi\Exceptions\UnexpectedValueException;
...
final class Container implements ContainerInterface
{
    ...
    public function build(string $id): mixed
    {
        if (! (isset($this->components[$id]) || class_exists($id))) {
            throw NotFoundException::new('Could not build %s.', [$id]);
        }
        ...
        if (is_array($component)) {
            ...
            throw UnexpectedValueException::new('Invalid component given');
        }
    ...
    public function instanciateArgs(object|string $object_or_class, array $args = []): ?object
    {
        ...
        throw UnexcpectedValueException::new('The class %s cannot be instantiated.', [$object_or_class::class]);
    }
    ...
    public function invokeFunctionArgs(ReflectionFunction|ReflectionMethod $func, array $args = [], ?object $object = null): mixed
    {
        ...
        if (! Reflector::checkType($type = $func->getReturnType(), $result)) {
            throw UnexpectedValueException::new('%s expected but %s provided.', [(string) $type ?: 'mixed', get_debug_type($result)]);
        }
